...
extern int z_impl_auxdisplay_custom_indicator_set(const struct device * dev, uint8_t index, bool enable);

__pinned_func
static inline int auxdisplay_custom_indicator_set(const struct device * dev, uint8_t index, bool enable)
{
#ifdef CONFIG_USERSPACE
	if (z_syscall_trap()) {
		union { uintptr_t x; const struct device * val; } parm0 = { .val = dev };
		union { uintptr_t x; uint8_t val; } parm1 = { .val = index };
		union { uintptr_t x; bool val; } parm2 = { .val = enable };
		return (int) arch_syscall_invoke3(parm0.x, parm1.x, parm2.x, K_SYSCALL_AUXDISPLAY_CUSTOM_INDICATOR_SET);
	}
#endif
	compiler_barrier();
...
#define auxdisplay_custom_indicator_set(dev, index, enable) ({ 	int syscall__retval; 	sys_port_trace_syscall_enter(K_SYSCALL_AUXDISPLAY_CUSTOM_INDICATOR_SET, auxdisplay_custom_indicator_set, dev, index, enable); 	syscall__retval = auxdisplay_custom_indicator_set(dev, index, enable); 	sys_port_trace_syscall_exit(K_SYSCALL_AUXDISPLAY_CUSTOM_INDICATOR_SET, auxdisplay_custom_indicator_set, dev, index, enable, syscall__retval); 	syscall__retval; })
#endif
#endif
